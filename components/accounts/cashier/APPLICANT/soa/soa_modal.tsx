...
import { addBreakDown, addGrant, checkSoa, getAcadYear, getESC, getGranted, getInfo, getRemainingBalance, getTuition, isLrnExist, prevDiscounts, searchSibling } from "@/src/actions/cashierAction";
...
const handleClose = () => {
  ...
  const handleGetInfoByLrn = async () => {
    ...
    if (check.length > 0) {
      setLrnLoading(false);
      toast.error("This student already has been assessed.");
    return
    }
    ...
    if (!info[0].gradeLevel) {
    throw new Error("Missing grade level");
}
    const tuition = await getTuition(info[0].gradeLevel);
    setTuition(tuition?.tuitionBase ?? 0);
    setMiscellaneous(tuition?.miscellaneous ?? 0);
              ...
              <article  className="flex flex-col gap-8 py-5">
                  ...
                  <div className="flex flex-col gap-5 bg-gray-100/50 shadow-lg border-2 rounded-lg p-4">
                    <table className="w-full text-sm">
                      <tbody>
                          ...
                          {acad !== "N/a" && (
                          <tr>
                              <td className="py-2 text-red-600">Less: {acad} Discount {acad === "With Honor" ? "(20%)" : acad === "With High Honor" ? "(50%)" : acad === "With Highest Honor" ? "(75%)" : ""}</td>
                              <td className="text-right text-red-600">-₱{acad === "With Honor" ? Math.ceil((Number(tuition) + Number(miscellaneous)) * 0.20) : acad === "With High Honor" ? Math.ceil((Number(tuition) + Number(miscellaneous)) * 0.50)  : acad === "With Highest Honor" ? Math.ceil((Number(tuition) + Number(miscellaneous))  * 0.75)  : 0}</td>
                          </tr>
                          )}
                          {sibling === "Yes" && (
                          <tr>
                          ...
                          <td className="text-right">₱ {Number(tuition) + Number(miscellaneous) - (escGrantee === "Yes" ? 9000 : 0) - (acad === "With Honor" ? Math.ceil((Number(tuition) + Number(miscellaneous)) * 0.20) : acad === "With High Honor" ? Math.ceil((Number(tuition) + Number(miscellaneous)) * 0.50)  : acad === "With Highest Honor" ? Math.ceil((Number(tuition) + Number(miscellaneous))  * 0.75)  : 0) - (sibling === "Yes" ? 500 : 0) - Number(other_discount )}</td>
                ...
                <section className="w-full text-center">
                  <p className="text-sm text-red-600 mb-3 font-medium">
                    Please double-check details before submitting.
                  </p>
                </section>
