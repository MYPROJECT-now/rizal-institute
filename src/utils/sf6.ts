...
// === Learning Progress & Achievement Data ===
const gradeRanges = [
  { key: 'Below 75', label: 'Did Not Meet Expectations (74 and below)' },
  { key: '75-79', label: 'Fairly Satisfactory (75-79)' },
  { key: '80-84', label: 'Satisfactory (80-84)' },
  { key: '85-89', label: 'Very Satisfactory (85-89)' },
  { key: '90-100', label: 'Outstanding (90-100)' },
];


const gradeLevels: GradeKey[] = ['7', '8', '9', '10']; // Grades to loop through
const rowStart = 15; // Starting row for grade ranges

gradeRanges.forEach((range, i) => {
  const row = rowStart + i;
  let colStart = 5; // E column is the first male column

  // Insert range label in column B


  // Fill in male/female/total for each grade
  gradeLevels.forEach((g) => {
    const male = sf6Data[g]?.maleGrades?.[range.key] || 0;
    const female = sf6Data[g]?.femaleGrades?.[range.key] || 0;
    ...
    for (let c = colStart; c <= colStart + 2; c++) {
      ...
      sheet.getCell(row, c).font = { size: 11, name: "Arial Narrow" };
    }
  ...
  const totalMale = gradeLevels.reduce(
    (sum, g) => sum + (sf6Data[g]?.maleGrades?.[range.key] || 0),
    0
  );
  const totalFemale = gradeLevels.reduce(
    (sum, g) => sum + (sf6Data[g]?.femaleGrades?.[range.key] || 0),
    0
  );

  sheet.getCell(row, 17).value = totalMale; // Q
  sheet.getCell(row, 18).value = totalFemale; // R
  sheet.getCell(row, 19).value = totalMale + totalFemale; // S

  // Apply borders and alignment for TOTAL
  [17, 18, 19].forEach((c) => {
    sheet.getCell(row, c).border = {
      top: { style: "thin" },
      left: { style: "thin" },
